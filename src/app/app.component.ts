...
@Component({
    ...
    imports: [MatButtonModule, InputComponent, OutputComponent, ModelComponent, EffectComponent, EqualityComponent]
})
export class AppComponent {
  monSignal: WritableSignal<number> = signal<number>(0);
  monComputedSignal: Signal<string> = computed(() => this.monSignal() % 2 === 0 ? 'Hello' : 'World');
  ...
  emittedSignal(value: string) {
    console.log(`handleEmittedSignal = ${value}`);
  }
